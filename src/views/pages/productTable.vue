<template>
  <div class="page-container common-css">
    <div class="page-main">
          ...
          <div class="menu-right">
            <el-form-item class="menu-btn">
              <el-button type="danger"
                         ...
                         @click="deleteTable"
                         size="small"
                         v-if="authInfo.remove == 1">删除</el-button>
              <el-button type="primary"
                         icon="el-icon-circle-plus"
                         @click="addTable"
                         size="small"
                         v-if="authInfo.inser == 1">新增</el-button>
              <el-button type="warning"
                         icon="el-icon-upload"
                         @click="showTableVisible()"
                         size="small"
                         v-if="authInfo.inser == 1">导入</el-button>
      ...
      <el-table :data="tableData"
        ...
        <el-table-column prop="prodname"
                         label="产品名称" />
        <el-table-column prop="chnnlcode"
                         label="一级类别编码" />
        <el-table-column prop="chnnlname"
                         label="一级类别名称" />
        <el-table-column prop="categorycode"
                         label="二级类别编码" />
        <el-table-column prop="categoryname"
                         label="二级类别名称" />
        <el-table-column prop="calcode"
                         label="统计口径编码" />
        <el-table-column prop="calcode2"
                         label="统计口径编码2" />
        <el-table-column prop="proddesc"
                         label="产品描述" />
        <el-table-column label="操作"
                         width="80"
                         v-if="authInfo.updat == 1">
      ...
      <div class="dialog-top">
        <div class="dialog-left">
          ...
          <el-button type="primary"
                     ...
                     :disabled="!checkPass">确定</el-button>
        </div>
        <div class="dialog-right">
          <el-button type="success"
                     icon="el-icon-download"
                     size="small"
                     @click.native="downloadExcel">模板下载</el-button>
        </div>
      </div>
    ...
    <el-dialog :title="modalInfo.title"
      ...
      <el-form :model="ruleForm"
        ...
        <el-form-item class="menu-btn"
                      label="产品编码"
                      prop="prodcode">
          <el-input v-model="ruleForm.prodcode"
                    placeholder="请输入产品编码"
                    :disabled="modalInfo.title == '修改框'"></el-input>
        </el-form-item>
        <el-form-item class="menu-btn"
                      label="产品名称"
                      prop="prodname">
        ...
        <el-form-item class="menu-btn"
                      label="一级类别编码"
                      prop="chnnlcode">
          <el-input v-model="ruleForm.chnnlcode"
                    placeholder="请输入一级类别编码"></el-input>
        </el-form-item>
        <el-form-item class="menu-btn"
                      label="一级类别名称"
                      prop="chnnlname">
          <el-input v-model="ruleForm.chnnlname"
                    placeholder="请输入一级类别名称"></el-input>
        </el-form-item>
        <el-form-item class="menu-btn"
                      label="二级类别编码"
                      prop="categorycode">
          <el-input v-model="ruleForm.categorycode"
                    placeholder="请输入二级类别编码"></el-input>
        </el-form-item>
        <el-form-item class="menu-btn"
                      label="二级类别名称"
                      prop="categoryname">
          <el-input v-model="ruleForm.categoryname"
                    placeholder="请输入二级类别名称"></el-input>
        </el-form-item>
        <el-form-item class="menu-btn"
                      label="产品描述"
                      prop="proddesc">
          <el-input v-model="ruleForm.proddesc"
                    placeholder="请输入产品描述"></el-input>
        </el-form-item>
        <el-form-item class="menu-btn"
                      label="统计口径1"
                      prop="calcode">
          <el-input v-model="ruleForm.calcode"
                    placeholder="请输入统计口径"
                    :disabled="modalInfo.title == '修改框'"></el-input>
        </el-form-item>
        <el-form-item class="menu-btn"
                      label="统计口径2"
                      prop="calcode2">
          <el-input v-model="ruleForm.calcode2"
                    placeholder="请输入统计口径"
                    :disabled="modalInfo.title == '修改框'"></el-input>
        </el-form-item>
      </el-form>
      <div class="btn-group">
        ...
        <el-button type="primary"
                   size="small"
                   @click="modalInfo.show = false">取消</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  data () {
    return {
      ...
      menuOptions: [{ name: '产品编码', value: 0 }, { name: '产品名称', value: 1 }],
      tableData: [],
      rules: {
        prodcode: [
  ...
  methods: {
    exportTable () {
      ...
      params[key2] = ''
      download(this.baseUrl + '/IopProdDTableController/downProdInfo',
        params, 'IOP产品维表'
      )
    },
    downloadExcel () {
      ...
      //   window.location.href = this.baseUrl + '/IopProdDTableController/downloadProdExcel'
      // } else {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          if (this.modalInfo.title == '新增框') {
            apiSend.prodInsertProdInfo({ data: this.ruleForm }).then((res) => {
            ...
            apiSend.prodUpdateProdInfo({ data: this.ruleForm }).then((res) => {
              this.modalInfo.show = false;
    ...
    deleteTable () {
      ...
      this.$confirm('进行删除操作, 是否继续?', '提示', {
        ...
        apiSend.prodRemoveProdByCode({ data: array }).then((res) => {
    ...
    handleDelete (index, row) {
      this.$confirm('进行删除操作, 是否继续?', '提示', {
        ...
        apiSend.prodRemoveProdByCode({ data: params }).then((res) => {
          this.$message.success('删除成功!')
          this.findTable()
        }).catch((err) => {
          this.$message.error(err.data.msg)
    ...
    handleEdit (index, row) {
      let { operationtime, operationuser, ...result
      } = row
      let self = this;
      this.$nextTick(() => {
        this.$refs['ruleForm'].resetFields();
